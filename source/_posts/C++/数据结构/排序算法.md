...
/*4、快速排序*/
void QuickSortR(RecType R[],int s,int t) //对R[s]至R[t]的元素进行快速排序
{
    int i=s,j=t;
    RecType tmp;
    if (s<t)                //区间内至少存在两个元素的情况
...
/*计算排序时间*/
long Sort(RecType R[],int n,void func(RecType[],int)){
    long beginTime,endTime;
    RecType R1[MaxSize];
    for(int i=0;i<n;i++)
        R1[i].key=R[i].key;
...
RecType R[MaxSize];//全局变量，避免爆栈

int main(){

    GetLargeData(R,MaxSize);//100005条
    printf("1、希尔排序时间：  %ld\n",Sort(R,MaxSize,ShellSort));
    printf("2、插入排序时间：  %ld\n",Sort(R,MaxSize,InsertSort));
    printf("3、冒泡排序时间：  %ld\n",Sort(R,MaxSize,BubbleSort1));
    printf("4、快速排序时间：  %ld\n",Sort(R,MaxSize,QuickSort));
    printf("5、堆排序时间：    %ld\n",Sort(R,MaxSize,HeapSort));
    printf("6、归并排序时间：  %ld\n",Sort(R,MaxSize,MergeSort));
    printf("7、基数排序时间：  %ld\n",Sort1(R,MaxSize));
    system("pause");
    return 0;
}
...
![](https://gitee.com/yuexiliuli/MyMarkdownImage/raw/master/img/3245434657653454657565.png)

![img](https://gitee.com/yuexiliuli/MyMarkdownImage/raw/master/img/3245434657653454657565.png)